...
## 2026-06-08 — Cross-Site Piece/Exhibit Embed Origin Preservation Fix

### Trigger
The owner generates "art piece" and "exhibit" iframe embed codes from one CreatrWeb deployment and pastes them into posts/pages on a different deployment (cross-posting between their own multiple sites). Pasted external embeds were silently rewritten to point at the *receiving* site's own canonical origin (observed: `platform.creatrweb.com`) instead of the originating site — breaking the embed, since no piece with that ID exists on the receiving site. This defeated the stated purpose of the iframe-embed feature: embedding content from a different site into this one.

### Root Cause
Three separate pieces of code shared the same flaw — each treated *any* URL whose path matched `/embed/pieces/<id>` or `/immersive/exhibits/<slug>` as "ours" and unconditionally substituted the current site's canonical origin, discarding the original host:
1. `normalizePieceEmbedSrc()` in `embed-utils.ts` — runs when the "Insert embed" dialog parses pasted code; collapsed any matching absolute URL to a relative path, destroying the origin at paste time (the actual entry point of the bug).
2. `normalizePieceEmbedSrc()` / `normalizeExhibitEmbedSrc()` in `lib/content-normalization.ts` — ran on editor load, save, and render; refilled the now-origin-less relative path with the *current* site's canonical origin (or the hardcoded `https://platform.creatrweb.com` fallback from `getCanonicalOrigin()` when `ALLOWED_ORIGINS`/`PUBLIC_SITE_URL` aren't set).
3. `normalizePieceUrlsInHtml()` in `routes/feeds.ts` — same flaw in feed/export generation (export.json, feed.xml, feed.json), via a regex that matched and discarded *any* absolute host.

This contradicted the precedent already established by `extractPieceEmbedMeta()` in `lib/immersive-view.ts`, which deliberately preserves `pieceOrigin: url.origin` "so cross-posted VR links point back to the site that owns the piece, not the current site." Note: generic external embeds (YouTube, CodePen, etc.) were never affected — `sanitizeRichHtml()` already passes `https://` iframe srcs through untouched; only same-path-shaped piece/exhibit embeds were being clobbered.

### Decisions Confirmed
- All three rewrite sites now apply the same narrowed condition: only replace the origin when the source URL is **relative** (no host at all — definitely ours) or its host is a **local-dev placeholder** (`localhost` / `127.0.0.1`, any port — the actual "don't leak dev URLs into production" scenario the code was meant to guard against). Any other absolute `https://` host is preserved verbatim as an intentional cross-site reference.
- `normalizePieceUrlsInHtml` in `feeds.ts` was changed from non-exported to `export`ed so it can be unit-tested directly, matching the existing pattern of exported helpers (`buildAtom`, `siteScope`, etc.) in that file.
- A pre-existing broken test ("normalizes pasted piece embeds to the live piece URL" in `RichPostEditor.test.tsx`) was fixed alongside this work — it asserted on a `window.prompt`-based interaction that the real `EmbedDialog` flow doesn't use (it's a controlled-textarea dialog), so the assertion was never actually exercised. Replaced with the correct type-into-textarea-and-click-Insert interaction and corrected expected values to match actual `parseIframeEmbed` behavior.

### Files Changed
- `artifacts/microblog/src/components/post/embed-utils.ts` — `normalizePieceEmbedSrc()` now preserves foreign-host absolute URLs instead of collapsing them to relative paths.
- `artifacts/microblog/src/lib/content-normalization.ts` — `normalizePieceEmbedSrc()` and `normalizeExhibitEmbedSrc()` gained the same local-dev-or-relative guard via a shared `isLocalDevHost()` helper.
- `artifacts/api-server/src/routes/feeds.ts` — `normalizePieceUrlsInHtml()` rewritten to capture and conditionally preserve the source host; exported for testability.
- `artifacts/microblog/src/components/post/__tests__/RichPostEditor.test.tsx` — fixed the broken piece-embed-normalization test and added a sibling case for cross-site preservation.
- New: `artifacts/microblog/src/lib/__tests__/content-normalization.test.ts` — direct coverage of `normalizePieceEmbedSrc`, `normalizeExhibitEmbedSrc`, and `normalizePieceEmbedUrls` across relative, local-dev, cross-site, and generic-external cases.
- New: `artifacts/api-server/src/routes/feeds.normalize.test.ts` — direct coverage of `normalizePieceUrlsInHtml` across the same cases.

### Outcome
- Pasting a piece or exhibit embed code generated by a different CreatrWeb deployment now keeps it pointed at the originating site, in the editor, on save, on render, and in feed exports.
- Embeds authored against a stale `localhost:PORT` origin (e.g. carried over from local development) still get corrected to the current site's canonical origin, preserving the original fix's intent.
- Generic external iframe embeds (YouTube, CodePen, etc.) were unaffected by this bug and remain unaffected by the fix — confirmed by running `sanitizeRichHtml()` directly against representative embed codes.
