...
describe("immersive-view helpers", () => {
  ...
  it("builds interactive image embeds with custom element wrappers", () => {
    ...
    expect(
      ...
      `<creatr-immersive-image ref="${encodedRef}" origin="http://localhost:3000"><iframe src="http://localhost:3000/immersive/images/${encodedRef}?embed=1&alt=Alt+text&title=My+Image" width="100%" style="width:100%;aspect-ratio:16 / 9;display:block;" title="My Image" frameborder="0" loading="lazy" allowfullscreen allow="fullscreen" sandbox="allow-scripts allow-same-origin"></iframe></creatr-immersive-image><script src="http://localhost:3000/embed.js" defer></script>`,
    );
  });
  ...
  it("builds interactive exhibit embeds with custom element wrappers", () => {
    expect(
      ...
      '<creatr-exhibit-wall slug="my-exhibit" origin="http://localhost:3000"><iframe src="http://localhost:3000/immersive/exhibits/my-exhibit?embed=1" width="100%" style="width:100%;aspect-ratio:16 / 9;display:block;" title="My Exhibit" frameborder="0" loading="lazy" allowfullscreen allow="fullscreen" sandbox="allow-scripts allow-same-origin allow-popups allow-popups-to-escape-sandbox allow-top-navigation-by-user-activation"></iframe></creatr-exhibit-wall><script src="http://localhost:3000/embed.js" defer></script>',
    );
  });
});
