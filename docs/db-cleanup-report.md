...
As of `2026-05-14`, the live app and the deployed Replit runtime expect the following tables to exist:

- `users`, `accounts`, `sessions`, `verification_tokens`
- `user_ai_vendor_settings`
- `art_pieces`, `art_piece_versions`
- `posts`, `comments`, `reactions`, `post_syndications`
...
- scheduled publishing (`posts.scheduled_at`, `posts.pending_platform_ids`, `posts.status` ∈ `{published, pending, draft, scheduled}`)
- POSSE syndication tracking (`post_syndications`, `platform_connections`, `platform_oauth_apps`)
...
- [lib/db/src/migrate.ts](/Users/Fornesus/Code/fornesusart-platform/lib/db/src/migrate.ts:100)
