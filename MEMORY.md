...
2026-06-06 · EMBEDS · Resolved layout alignment and cutting-off issues for interactive embeds inside iframe modules (such as Hostinger Web Builder). When running inside an iframe, `embed.js` injects centering flex styles to the parent document's body. Web Component custom elements (`CreatrArtPiece`, `CreatrImmersiveImage`, and `CreatrExhibitWall`) use `:host` styles to fit responsively without overflowing (`max-width: 100%; max-height: 100%; aspect-ratio: 16/9; margin: auto;`). Added an iframe-escape redirection sequence to `<creatr-art-piece>`'s click handler on iPhone Safari, matching images/exhibits behavior by trying `window.open` and falling back to location redirection.

2026-06-06 · EMBEDS · Resolved interactive embed fullscreen regressions by making desktop/Android/iPad iframe embeds request native fullscreen inside the iframe (using `requestElementFullscreen` in `ImmersiveRouteShell.tsx`) rather than passing a postMessage to the parent wrapper. This preserves synchronous user-activation, ensuring native element fullscreen is successful and escapes host-page transform/clipping traps. On iPhone/iOS (where native element fullscreen is unsupported), the parent wrapper fallback toggles a CSS fixed overlay (`100dvh`), and `embed.js` dynamically relocates the custom element to `document.body` while fullscreen is active to escape transformed host-page containers. If running inside a nested iframe module on iPhone (e.g. Hostinger Web Builder), `ImmersiveRouteShell.tsx` detects this (`window.parent !== window.top`) and escapes the trapping iframe container via a top-level location redirect or new tab fallback. Added `margin: 0 auto` to `:host` custom elements and copy snippets to properly center embeds on desktop pages. Added `:host(:fullscreen)` and `aspect-ratio: auto !important` styles to `<creatr-art-piece>`, and registered resize event listeners to automatically update WebGL (Three.js) and 2D canvas (c2) dimensions, guaranteeing correctly centered and scaled layouts upon fullscreen activation.
