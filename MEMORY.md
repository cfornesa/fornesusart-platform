...
2026-06-05 · IMMERSIVE · Interactive gallery embeds now have an Apple-mobile Safari-specific escape path: iPhone/iPad Safari piece embeds open the existing canonical immersive piece route in a new tab with `appleMobileEmbed=1`, and that flagged immersive view replaces `Back` with `Home` targeting `PUBLIC_SITE_URL` (or the canonical origin fallback). Android and desktop Safari keep the existing immersive/fullscreen behavior. New copied interactive embed code includes `allow-top-navigation-by-user-activation`, and first-party post rendering normalizes stored piece/exhibit iframes to that sandbox at render time.
    [Verified from `ImmersiveRouteShell.tsx`, `immersive-piece.tsx`, `meta-injection.ts`, `immersive-view.ts`, the Apple-mobile Safari detector, and focused immersive helper/shell tests.]
