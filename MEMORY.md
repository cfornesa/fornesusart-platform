...
2026-06-05 · IMMERSIVE · Interactive gallery embeds now treat fullscreen fallback as capability-based: if an embedded immersive piece cannot enter native fullscreen (notably iPhone Safari), the control promotes the viewer into the canonical immersive route in the same tab so browser Back returns to the source page. New copied interactive embed code adds `allow-top-navigation-by-user-activation` to the outer iframe sandbox, and first-party post rendering now normalizes stored piece/exhibit iframes to that sandbox at render time.
    [Verified from `ImmersiveRouteShell.tsx`, `PostContent.tsx`, `immersive-view.ts`, and focused immersive/post-content tests.]
